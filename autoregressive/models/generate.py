...
def decode_n_tokens(
    ...
    window_parameter: int = 64, window_type: str = "1d", **sampling_kwargs):
    ...
    for i in range(num_new_tokens):
        with torch.backends.cuda.sdp_kernel(enable_flash=False, enable_mem_efficient=False, enable_math=True): # Actually better for Inductor to codegen attention here
            ...
            next_token, next_prob = decode_one_token(
                ...
                local_guidance_scale, window_parameter, window_type, **sampling_kwargs
            )
            input_pos += 1
...
def generate(model, cond, max_new_tokens, emb_masks=None, cfg_scale=1.0, cfg_interval=-1, 
    ...
    next_token = prefill(model, cond_combined, input_pos, cfg_scale, local_guidance_scale, window_parameter, window_type, **sampling_kwargs)
    seq[:, T:T+1] = next_token

    input_pos = torch.tensor([T], device=device, dtype=torch.int)
    generated_tokens, _ = decode_n_tokens(model, next_token, input_pos, max_new_tokens-1, cfg_scale, cfg_interval, local_guidance_scale, window_parameter, window_type, **sampling_kwargs)
